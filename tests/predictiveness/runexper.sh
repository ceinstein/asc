...
create(){
	...
	do
		...
		cd ../ubasicexper/tests/predictiveness
		...
		do
		    ...
		    if [ -f ../ubasicexper/tests/predictiveness/$filetype/$exec/nets/$executable\_$line\_$window.net ];
		    then
			./asc -a $line/$window ../ubasicexper/tests/predictiveness/$filetype/$exec/nets/$executable\_$line\_$window.net ../ubasicexper/tests/executables/$executable &> ../ubasicexper/tests/predictiveness/$executable\_$window/$line.txt;
		    ...
		    printf $line" " >> ../ubasicexper/tests/predictiveness/$filename.txt;
		    counter=$((counter + 1))
		    
		    #Takes the last line in the PC file and puts the excited bit count into the data file                                                                                           
		    let num=$(grep "+" ../ubasicexper/tests/predictiveness/$executable\_$window/$line.txt -c);
		    let den=$(tail -1 ../ubasicexper/tests/predictiveness/$executable\_$window/$line.txt | cut -f 1 | xargs);
		    printf "%f " "$(echo $num / $den | bc -l)" >> ../ubasicexper/tests/predictiveness/$filename.txt;
		    
		    printf $window" " >> ../ubasicexper/tests/predictiveness/$filename.txt;

		    printf $exec" " >> ../ubasicexper/tests/predictiveness/$filename.txt;
		    
		    printf $counter"\n" >> ../ubasicexper/tests/predictiveness/$filename.txt;
		...
		gnuplot <<- EOF
                    ...
                    set nokey
                    set yrange[0:]
                    set title "PCs vs Predictiveness for $filename with WS $window"
                    set xlabel "PCs"
